...
import ru.g000sha256.reduktor.demo.network.ApiError

class MainErrorProvider(private val resources: Resources) {

    val activityNotFoundError: String
        get() = resources.getString(R.string.main_error_message_activity_not_found)

    fun getNetworkError(throwable: Throwable?): String {
        if (throwable is ApiError) {
            return throwable.error
        }
        return resources.getString(R.string.main_error_message_network)
    }
